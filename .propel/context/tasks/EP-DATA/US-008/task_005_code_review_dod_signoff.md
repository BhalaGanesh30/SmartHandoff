...
- [ ] RLS policy `audit_immutable` active on `audit_log` in dev, staging, and prod environments
- [ ] Three database roles created with correct least-privilege grants: `app_write`, `audit_writer`, `compliance_reader`
- [ ] `HIPAAAuditMiddleware` registered in `main.py` and writing records for all PHI endpoints
- [ ] Integration tests: all 8 test cases pass in Cloud Build CI
- [ ] pg_cron nightly retention job scheduled (verified via `SELECT * FROM cron.job` in Cloud SQL)
- [ ] Security Engineer sign-off recorded on the GitHub PR
- [ ] No PHI field values in any `audit_log` column — confirmed by `grep` and code review
- [ ] `alembic downgrade -1` tested reversible for all three migrations (0001, 0002, 0003)
